...
sub init {
	...
	$self->{data}->setDB($self->const->get('Md_db'))->setColl($self->const->get('Md_start_coll'));

	$self->log->info('Start Coll: initialized.');
	return $self;
}
...
sub getUserByStartN {
	my ($self, $env, $startN) = @_;

	# Check user
	my $resp = $self->data->get( undef, {startN => $startN} );
