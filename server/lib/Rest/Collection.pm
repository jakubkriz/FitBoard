...
sub GET {
	...
	my $projId = $env->{'hmeter.projectid'};
	my $collId = $env->{'hmeter.collectionid'};

	my $db = $self->data;
	$self->data->setDB($projId);
	my @colls = $self->data->getColls($projId);
	...
	if (!$collId){
		...
		foreach my $coll (@colls) {
			next if $coll =~ /system\./;
			push @$links, {
				href => $self->refToUrl("Rest::Collection::Id",{'hmeter.projectid'=>$projId, 'hmeter.collectionid' => $coll, 'hmeter.apiversion'=>$env->{'hmeter.apiversion'}}),
				title => $coll,
				rel => 'Rest::Collection::Id',
			}
		}
		return {
			link => $links
		...
		if (grep($_ eq $collId, @colls)){
			$self->data->setColl($collId);
			my $info = $self->data->get($self->const->get('Md_coll_default'));
			$info->{link} = $self->getLinksForResource("Rest::Collection::Id", {'hmeter.projectid'=>$projId, 'hmeter.collectionid' => $collId, 'hmeter.apiversion'=>$env->{'hmeter.apiversion'}}),;
			return $info;
